...
def sce():
    ...
    x_grid = [sample_exponential_dist(1.0 / x_0) for _ in range(n_part)]
    x_grid.sort()
    r_grid = [math.cbrt(x * THREE_FOURTH / PI) for x in x_grid]

    # Create the initial droplet array
    droplets = [Droplet.new(xi_i, radius) for radius in r_grid]
    m_grid = [d.mass for d in droplets]
    ...
    bin_edge_exponents = linspace(0.0, math.log10(r_max_bin), nr + 1)
    bin_edges = [10.0**exponent for exponent in bin_edge_exponents]
    bin_mids = [
        0.5 * (left + right) for left, right in zip(bin_edges[:-1], bin_edges[1:])
    ]
